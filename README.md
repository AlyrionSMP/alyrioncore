...
- **Directional input ports**: the custom model has two dedicated ports — the **power terminal** on one side (east of the model) and the **water flange** on the other (west). The capabilities are gated per side, so Power Grid cables only connect to the power port and Create pipes only to the water port.
