...
| **Pride Cape** | `pride.png` | **Not Buyable** | Complete *"United We Stand"* (party of 4+ via Open Parties and Claims) | The Progress Pride flag worn hoist-up: six rainbow stripes behind the nested black/brown and trans chevron, earned by partying up with at least 4 players in an OPAC party. |
...
- On vacuum worlds, a sealed room is breathable **only once its oxygen fill reaches 100%**. Pressurization is gradual: **0.5 s per interior block per running Oxygen Generator** — generators stack linearly, so **two generators double the speed, three triple it**, etc. (a 300-block greenhouse takes 150 s with one generator, 75 s with two).
- While a room is still pressurizing, **"⚠ Pressurizing — X%…"** is shown and the air still drains — don't flip the switch and wander off with your helmet off. The fill is time-based and keeps progressing even while you're outside; it is stored per room, so a power blip only costs you part of the fill (refilling resumes where it left off).
- A sealed room with an **unpowered / missing** generator is *not* breathable: the oxygen drains away and you drown — the generator is the difference between a habitat and a tomb.
...
- In-game feedback: an actionbar shows **"✔ Pressurized habitat detected — breathing"** once a room is fully filled, **"⚠ Pressurizing — X%…"** while it fills (updated at 25% steps), **"⚠ Habitat sealed — no oxygen generator running!"** when the room is sealed but starved of power, and **"⚠ Habitat breached — depressurizing!"** the moment a sealed habitat loses pressure (airlock opened, wall broken — the room vents to vacuum and must refill from scratch).

### The Oxygen Generator (`oxygen_generator`)

The **heart of every habitat**: a purpose-built machine (fully custom model & textures — nothing reused or stretched from other blocks) with a stepped two-tier body on rubber feet, an overhanging top cap, a raised vent ring holding the animated spinning impeller (rendered by `OxygenGeneratorBlockEntityRenderer`), a gauge pod and instrument buttons, and two **identical large connector ports** on either side that differ only by their symbol — a lightning bolt for power, a droplet for water. It is what makes a sealed room breathable.
...
- **Inputs**: the model marks a **power terminal** (one side) and a **water flange** (the other) as the intended inputs, but **both capabilities are exposed on every face** — FE cables from any mod and Create pipes attach on any side, so no per-mod compat is needed. Connect wherever is convenient.
- **Dual-consumption drain**: while it has *both* FE and water the machine runs, consuming **4 FE/t** and **2 mB/t of water** (~80 FE/s + ~40 mB/s; both buffers last ~3.3 minutes). Let either run dry and the room's oxygen starts draining — breathability is lost almost immediately, but the remaining fill is kept, so restarting the generator refills faster than from scratch.
- **Active state**: while running the block emits light level 8, the front dial glows and the vent fan spins; when either buffer runs dry it goes dark and still.
...
- **Overworld Space Program**: everything needed to reach space is obtainable from the Overworld — Rocketnautics' *engine nozzle*, *engine cycle pipes*, *thruster mount* and *hose anchor* gain crafting recipes from plain overworld materials (iron, copper, obsidian), plus the *Space Helmet* (iron + glass), and titanium ore already generates in Overworld biomes, so the whole rocket line (titanium → Create machines → engines) never requires leaving the homeworld. (Rocketnautics' *fluid hose* item is not registered by the mod — no recipe for it.)
...
| **Reinforcement Plates** | `iron_reinforcement_plate` · `diamond_reinforcement_plate` · `meteoric_iron_reinforcement_plate` · `netherite_reinforcement_plate` | Defense | Forged plates that bolt onto any full solid block (a normal full-cube model), multiplying its mining durability 3×–100× (see [Block Reinforcement](#-block-reinforcement)). |
...
Reinforce any full solid block so it survives repeated mining — the fortress-block mechanic:

- **Plates**: four tiers, each crafted **2×2 → 8 plates** — **Iron (3×)**, **Diamond (10×)**, **Meteoric Iron (30×)** and **Netherite (100×)** hit multipliers.
- **Applying**: right-click an eligible block with a plate. **Eligible** means breakable, no block entity, and a **full solid block with a normal block model**: its collision shape must fill the whole 1×1×1 cell and it must render as a regular model. That refuses torches, doors, levers, buttons, slabs, stairs, fences, carpets, rails and the like — a partial or offset model leaves the plate frame floating and the hitbox wrong — while full cubes that are merely transparent, such as **glass**, stay eligible. The block is replaced by a `reinforced_block` wrapper that stores the original state — it keeps its own look (a riveted plate frame on air-facing sides, with the original block drawn inside by a block-entity renderer), so a reinforced wall still looks exactly like the wall you built.
...
- `generate_capes.py`: Regenerates all 64x32 custom capes with anti-aliased pixel art (includes the Grim Cape and the Pride Cape, whose Progress-flag chevron geometry is sampled from the flag's own SVG).
