...
- **Inputs**: the model marks a **power terminal** (one side) and a **water flange** (the other). Power Grid cables attach at the power terminal; **Create pipes attach on any face** — the water flange is the marked water input, but the fluid capability is exposed on every side so pipes always connect.
