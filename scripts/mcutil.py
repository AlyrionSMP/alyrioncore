...
def banding_detect(img, min_run=4, min_step=6):
    ...
    Walks every row and every column separately (so non-square images such as
    64x32 capes work too), collecting maximal runs of same-hue pixels whose
    luma steps at least `min_step` in one direction each pixel.

    Returns a list of (orientation, index, start, end, direction).
    """
    h, w = len(img), len(img[0])
    runs = []
    for orient in ('row', 'col'):
        outer, inner = (h, w) if orient == 'row' else (w, h)
        for i in range(outer):
            line = [img[i][j] for j in range(inner)] if orient == 'row' \
                else [img[j][i] for j in range(inner)]

            def close(start, end, direction):
                if start is not None and direction and end - start + 1 >= min_run:
                    runs.append((orient, i, start, end,
                                 'up' if direction > 0 else 'down'))

            start, direction, prev, prev_hue = None, 0, None, None
            for j, px in enumerate(line):
                if px[3] == 0:
                    close(start, j - 1, direction)
                    start, direction, prev, prev_hue = None, 0, None, None
                    continue
                v = luma(px)
                hue = rgb2hsv(px[:3])[0]
                if prev is None:
                    start, prev, prev_hue = j, v, hue
                    continue
                step = v - prev
                same_hue = min(abs(hue - prev_hue), 360 - abs(hue - prev_hue)) < 25
                d = 0 if abs(step) < min_step else (1 if step > 0 else -1)
                if d == 0:
                    close(start, j - 1, direction)
                    start, direction = j, 0
                elif not same_hue or (direction and d != direction):
                    close(start, j - 1, direction)
                    start, direction = j - 1, d
                else:
                    direction = d
                prev, prev_hue = v, hue
            close(start, inner - 1, direction)
    return runs[:12]
...
def write_mosaic(path, imgs, scale=8, gap=2, bg=(32, 32, 36, 255)):
    ...
    Cells size themselves to the largest image, so 64x32 capes and other
    non-16x16 sheets tile as happily as block textures do.
    """
    cols = max(1, int(round(math.sqrt(len(imgs)))))
    rows = (len(imgs) + cols - 1) // cols
    cell_w = max(len(im[0]) for im in imgs) * scale + gap
    cell_h = max(len(im) for im in imgs) * scale + gap
    out = new_img(cols * cell_w + gap, rows * cell_h + gap, bg)
    for i, img in enumerate(imgs):
        sx, sy = (i % cols) * cell_w + gap, (i // cols) * cell_h + gap
...
def write_wall(path, img, scale=8):
    ...
    h, w = len(img), len(img[0])
    out = new_img(w * scale * 3, h * scale * 3, (40, 40, 44, 255))
    big = _scale_nearest(img, scale)
    for ty in range(3):
        for tx in range(3):
            for y in range(h * scale):
                for x in range(w * scale):
                    out[ty * h * scale + y][tx * w * scale + x] = big[y][x]
    write_png(path, out)
    return path
